...
-(void)showSuper:(UIView*)aview icon:(NSString*)icon{
    ...
    if ([UIImage imageNamed:icon].size.width > self.iconView.mj_w) {
        
        self.iconView.mj_size = [UIImage imageNamed:icon].size;
    }
}
-(void)showCenterWithSuperView:(UIView*)aview icon:(NSString*)icon{
    [self showSuper:aview icon:icon];
    self.frame= CGRectMake(0, 0, aview.mj_w, aview.mj_h);
}
